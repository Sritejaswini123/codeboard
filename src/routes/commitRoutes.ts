import factory from "../factory.js";


import {avergaeCommitsHandlers, createCommitHandlers, deleteCommitByIdHandlers, getAllCommitsHandlers, getCommitByIdHandlers, statusCountHanders, updateCommitByIdHandlers } from "../handlers/commitHandlers.js";

const commitRoutes = factory.createApp();

commitRoutes.post("/save-commits", ...createCommitHandlers);
commitRoutes.get("/commits", ...getAllCommitsHandlers);
commitRoutes.get("/commits/:commit_id", ...getCommitByIdHandlers);
commitRoutes.put("/commits/:commit_id", ...updateCommitByIdHandlers);
commitRoutes.delete("/commits/:commit_id", ...deleteCommitByIdHandlers);

commitRoutes.get("/status-count", ...statusCountHanders);
commitRoutes.get("/average-commits", ...avergaeCommitsHandlers);



export default commitRoutes;
