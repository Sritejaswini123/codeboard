...
import { deleteFileFromS3, generateDownloadSignedUrl, generateSignedUploadUrl } from '../s3/publicDataServiceProvider';
import factory from '../factory';

//Generating 
export const uploadHandler = factory.createHandlers(async (c: Context) => {
  try {
    const { filename, contentType, size } = await c.req.json();
    if (!filename || !contentType || typeof size !== 'number') {
      return c.json(
        { success: false, error: 'Invalid request. Required: filename, contentType, size (number)' },
        BAD_REQUEST
      );
    }
...
export const getSignedDownloadUrlHandler = factory.createHandlers(async (c: Context) => {
...
export const deleteFileHandler =factory.createHandlers( async (c: Context) => {
  try {
