...
export const signUpHandler = factory.createHandlers(async (c: Context) => {
  const {
    email, password,first_name,phone, dob, doj,designation,
  } = await c.req.json();
...
//sign out the user
export const signOutWithSessionHandler = factory.createHandlers(async (c: Context) => {
  try {
    ...
    const sessionQuery = await pool.query(
      'SELECT id, token, "expiresAt" FROM session WHERE token = $1 AND "expiresAt" > NOW()',
      [session_token]
    );
...
export const signOutWithSessionHandler2 = factory.createHandlers(async (c: Context) => {
  try {
    const { session_token } = await c.req.json();
    if (!session_token) {
      throw new conflictException("Session token is required");
    ...
    const pool = auth.options.database;
    const sessionQuery = await pool.query(
      'SELECT id, token, "userId", "expiresAt" FROM session WHERE token = $1 AND "expiresAt" > NOW()',
      [session_token]
    );
...
export const signOutHandler = factory.createHandlers(async (c: Context) => {
  try {
    const { session_token } = await c.req.json();
