...
export const vCreateCommit = z.object({
  ...
  //   }),
  date: z.string()
  .regex(/^\d{2}-[a-zA-Z]{3}-\d{2}$/, { message: "Date must be in DD-MMM-YY format" })
  .refine(val => {
    const [day, month, year] = val.split('-');
    const validDay = parseInt(day, 10);
    const validYear = parseInt(year, 10);
    const validMonth = validMonths.includes(month.toLowerCase());
    return validDay >= 1 && validDay <= 31 && validYear >= 0 && validYear <= 99 && validMonth;
  }, { message: "Invalid date" }),
